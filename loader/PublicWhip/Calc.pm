# $Id: Calc.pm,v 1.12 2006/03/20 19:08:24 publicwhip Exp $
...
sub current_rankings {
    ...
    do_house_ranking($dbh, "commons");
    do_house_ranking($dbh, "lords");
}

sub do_house_ranking {
    my ($dbh, $house) = @_;
    ...
    if ($sth->rows == 0) {
        ...
        if ($sth->rows == 0) {
            PublicWhip::Error::log( "No MPs/Lords currently active have been found, change General Election date in code if you are coming up to one", "", ERR_IMPORTANT );
            return;
        }
    }
