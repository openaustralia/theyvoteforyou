  ...
  describe ".version_sentence" do
    ...
    context "changing policy vote" do
      ...
      context "change vote on policy" do
        let(:version) { double("version", item_type: "PolicyDivision", event: "update", whodunnit: 1, created_at: 1.hour.ago, changeset: { "vote" => %w[no aye] }, reify: double("policy_division", division_id: 5), policy_id: 3) }
