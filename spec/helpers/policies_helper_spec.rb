...
describe PoliciesHelper, :type => :helper do
  ...
  describe ".policies_list_sentence" do
    ...
    it { expect(helper.policies_list_sentence([policy1])).to eq '<a href="/policy.php.1">A nice policy</a>' }
    it { expect(helper.policies_list_sentence([policy2])).to eq '<a href="/policy.php.2">A provisional policy</a> <i>(provisional)</i>'}
    it { expect(helper.policies_list_sentence([policy3])).to eq '<a href="/policy.php.3">&lt;em&gt;A&lt;/em&gt; provisional policy</a> <i>(provisional)</i>'}
    it { expect(helper.policies_list_sentence([policy3])).to be_html_safe }
    it { expect(helper.policies_list_sentence([policy1, policy2])).to eq '<a href="/policy.php.1">A nice policy</a> and <a href="/policy.php.2">A provisional policy</a> <i>(provisional)</i>'}
  end
end
