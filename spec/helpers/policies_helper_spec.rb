...
describe PoliciesHelper, type: :helper do
  ...
  describe ".version_sentence_no_attribution" do
    before :each do
    ...
    context "create provisional policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Created draft policy “A new policy” with description “Oh yes!”.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "create policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Created policy “A new policy” with description “Oh yes!”.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "change name on policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Changed name from “Version A” to “Version B”.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "change description on policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Changed description from “Description A” to “Description B”.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "change status on policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Changed status to not draft.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "change everything on policy" do
      ...
      it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Changed name from “Version A” to “Version B”.</p><p class="change-action">Changed description from “Description A” to “Description B”.</p><p class="change-action">Changed status to draft.</p>' }
      it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
    end
    ...
    context "changing policy vote" do
      ...
      context "create vote on policy" do
        ...
        it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Added division <em><a href="/divisions/representatives/2001-01-01/2">blah</a></em>. Policy vote set to <span class="division-policy-statement-vote voted-aye">Yes (strong)</span>.</p>' }
        it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
      end
      ...
      context "remove vote on policy" do
        ...
        it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Removed division <em><a href="/divisions/representatives/2001-01-01/2">blah</a></em>. Policy vote was <span class="division-policy-statement-vote voted-no">No</span>.</p>' }
        it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
      end
      ...
      context "change vote on policy" do
        ...
        it { expect(helper.version_sentence_no_attribution(version)).to eq '<p class="change-action">Changed vote from <span class="division-policy-statement-vote voted-no">No</span> to <span class="division-policy-statement-vote voted-aye">Yes</span> on division <em><a href="/divisions/representatives/2001-01-01/2">blah</a></em>.</p>' }
        it { expect(helper.version_sentence_no_attribution(version)).to be_html_safe }
      end
    end
  end
