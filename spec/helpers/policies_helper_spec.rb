...
describe PoliciesHelper, :type => :helper do
  ...
  describe ".policies_list_sentence" do
    ...
    it { expect(policies_list_sentence([policy1])).to eq '<a href="/policy.php.1">A nice policy</a>' }
    it { expect(policies_list_sentence([policy2])).to eq '<a href="/policy.php.2">A provisional policy</a> <i>(provisional)</i>'}
    it { expect(policies_list_sentence([policy1, policy2])).to eq '<a href="/policy.php.1">A nice policy</a> and <a href="/policy.php.2">A provisional policy</a> <i>(provisional)</i>'}
  end
end
