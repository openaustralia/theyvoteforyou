...
describe PolicyPersonDistance, type: :model do
  describe ".category_with_not_enough" do
    ...
    it "distance 0.1" do
      ...
      expect(ppd.category_with_not_enough).to eq :for2
    end

    it "distance 0.3" do
      ppd = described_class.new(nvotessame: 3, distance_a: 0.3)
      expect(ppd.category_with_not_enough).to eq :for1
    end

    it "distance 0.5" do
      ppd = described_class.new(nvotessame: 3, distance_a: 0.5)
      expect(ppd.category_with_not_enough).to eq :mixture
    end

    it "distance 0.7" do
      ppd = described_class.new(nvotessame: 3, distance_a: 0.7)
      expect(ppd.category_with_not_enough).to eq :against1
    end

    it "distance 0.9" do
      ppd = described_class.new(nvotessame: 3, distance_a: 0.9)
      expect(ppd.category_with_not_enough).to eq :against2
    end

    it "distance 0.96" do
      ppd = described_class.new(nvotessame: 3, distance_a: 0.96)
      expect(ppd.category_with_not_enough).to eq :against3
    end
  end
end
