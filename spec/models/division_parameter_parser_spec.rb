...
describe DivisionParameterParser, type: :model do
  describe ".date_range" do
    it "gets range for complete date's format" do
      date_start, date_end, date_range = DivisionParameterParser.date_range("2017-12-20")
    ...
    it "gets range for year and month date's format" do
      date_start, date_end, date_range = DivisionParameterParser.date_range("2017-12")
    ...
    it "gets range for year only date's format" do
      date_start, date_end, date_range = DivisionParameterParser.date_range("2017")
    ...
    it "raise exception for invalid date" do
      expect { DivisionParameterParser.date_range("2017-13-01") }.to raise_error(ArgumentError)
    end

    it "raise exception for date in the wrong format" do
      expect { DivisionParameterParser.date_range("2017-12-011") }.to raise_error(ArgumentError)
    end
  end
end
