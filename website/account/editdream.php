...
if (user_isloggedin()) # User logged in, show settings screen
{
    ...
    $query = "select name, description, user_id from pw_dyn_rolliemp where rollie_Id = '$id'";
    ...
    else
    {
        ...
        if ($submit && (!$just_logged_in)) 
        {
            ...
            {
                $db = new DB(); 
                $ret = $db->query_errcheck("update pw_dyn_rolliemp set name='$name', description='$description' where rollie_id='$id'");

                if ($ret)
                {
                    $ok = true;
                    $feedback = "Successfully edited dream MP '" . html_scrub($name) . "'.  
                     To see the changes, go to <a href=\"../dreammp.php?id=$id\">your
                     dream MP's page</a>.";
