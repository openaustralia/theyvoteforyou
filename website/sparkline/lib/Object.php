 ...
 *
 * $Id: Object.php,v 1.1 2005/03/31 22:58:54 frabcus Exp $
 *
 */
...
define('DEBUG_ALL',   2048); // everything
...
class Object {
  ...
  function SetDebugLevel($level, $file = null) {
    ...
    if ($file != null) {
      if (!file_exists($file) ||
          !is_writable($file)) {
        die('error log file "$file" does not exist or is not writable to the web server user');
      } else {
        $this->logFile = $file;
        $LOGFILE       = $file;
      }
