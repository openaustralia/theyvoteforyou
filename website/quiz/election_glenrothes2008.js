...
function UpdateCItable(vevent)
{
    var hrefhash = location.href.lastIndexOf('#');
    var hrefpos = (hrefhash != -1 ? location.href.substring(hrefhash + 1) : "");
    //alert(hrefpos);
    var bhideunselected = document.getElementById("hideunselected").checked;
    ...
    for (var j = 0; j < issues.length; j++)
    {
        ...
        if (option.value == "00")   // nifty trick to avoid hacking the parseInt
            option.style.color = "#ff2222"; 
        else
            option.style.color = "black"; 

        var seletr = document.getElementById(Srowid);
        if ((imap[j] == 0) && bhideunselected)
        {
...
var unq = 0; 
function NotifyEvent(vevent)
{
    unq += 1; 
    var imim = document.createElement('img'); 
    var dpage = document.getElementById("dynamicpage"); 
    var evrand = document.getElementById("vrand"); 
    vrand = evrand.getAttribute('value'); 
    imim.setAttribute('src', '?vrand=' + vrand + '&vevent=' + vevent + '&unq=' + unq); 
    dpage.appendChild(imim); 
}
